...
const { zeusmenu } = require('./src/maiamenu')
...
const { maiamenu } = require('./database/maiamenu')
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:MAIA\n' //NAMA LU
            + 'ORG:Owner Bot;\n' // NAMA CONTACT OWNER
            + 'TEL;type=CELL;type=VOICE;waid=5574999250802:+55 74 99925-0802\n' // NOMER OWNER
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
blocked = ['']//NOMER BLOCK
limitawal = '50'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5574999250802@s.whatsapp.net"] // Nomer Owner
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5574999250802@s.whatsapp.net'//Nomer Owner
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = 'Maia//Nama Bot
                        const ownerName = 'Maia<3'//Nama Owner
